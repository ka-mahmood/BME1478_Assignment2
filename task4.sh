...
echo -------------------- PART FOUR --------------------
echo Checking that assignment-data is intact. As per TA suggestion, including file and folder comparison:
...
# echo This is used in the form "diff {option} ... {path 1} {path 2}". \
	...
	# to the first depth by default: "diff assignment-data assignment-data-proper" will only \
...
# echo Some of the options include "-r" for recursive, "-i" to ignore case, and "-y" to show \
	...
	# "diff -r assignment-data assignment-data-proper:", which uses the code 'r' to recursively also \
